...
class ButtonsApp(App, FloatLayout):
    ...
    def on_mouse_pos(self, instance, pos):
        ...
        if icon_active == 'maximize':
            ...
            if (pos[1] < 7 and pos[0] < Window.size[0] - 7) and (pos[1] < 7 and pos[0] > 7): 
            ...
            elif (pos[1] > 8 and pos[0] > Window.size[0] - 7) and (pos[1] < Window.size[1] - 8 and pos[0] > Window.size[0] - 7):
                Window.set_system_cursor('size_we') #west to east arrow for horizontal sizing
            #//TOP - North to South//
            elif (pos[1] > Window.size[1] - 7 and pos[0] < Window.size[0] - 7) and (pos[1] > Window.size[1] - 7 and pos[0] > 7):
                Window.set_system_cursor('size_ns') #north to south arrow for vertical sizing
            #//TOP RIGHT - Northeast to Southwest//
            elif pos[1] > Window.size[1] - 7 and pos[0] > Window.size[0] - 7:
                Window.set_system_cursor('size_nesw') #northeast to southwest for diagonal sizing
            #//TOP LEFT - Northwest to Southeast//
            elif pos[1] > Window.size[1] - 7 and pos[0] < 7:
                Window.set_system_cursor('size_nwse') #northwest to southeast for diagonal sizing
            #//LEFT SIDE - West to East//
            elif (pos[1] > 8 and pos[0] < 7) and (pos[1] < Window.size[1] - 8 and pos[0] < 7):
                Window.set_system_cursor('size_we') #west to east arrow for horizontal sizing
            #//BOTTOM LEFT - Northeast to Southwest//
            elif pos[1] < 7 and pos[0] < 7:
                Window.set_system_cursor('size_nesw') #northwest to southeast for diagonal sizing
            #//ANYTHING ELSE//
            else:
                Window.set_system_cursor('arrow') #normal arrow
    ...
    def on_touch_down(self, touch):
        if touch.button == 'left': #only run all this on a left click..
            ...
            if self.ids.s3.collide_point(touch.pos[0], touch.pos[1]): #if close button is clicked..
                ...
                Window.close()
            if touch.pos[1] > Window.size[1] - 30 and title_bar_button_clicked == 'N': #this will limit dragging of the window to only when the cursor is near the very TOP of the window..
            ...
            if icon_active == 'maximize' and title_bar_button_clicked == 'N':
                ...
                #//BOTTOM - North to South//
                if (touch.pos[1] < 7 and touch.pos[0] < Window.size[0] - 7) and (touch.pos[1] < 7 and touch.pos[0] > 7): 
                ...
                #//BOTTOM RIGHT - Northwest to Southeast//
                    ...
                    ButtonsApp.mouse_listener_active2 = 'Y'
                #//RIGHT SIDE - West to East//
                elif (touch.pos[1] > 8 and touch.pos[0] > Window.size[0] - 7) and (touch.pos[1] < Window.size[1] - 8 and touch.pos[0] > Window.size[0] - 7):
                    window_resize_feature = 'w_e_r' #west to east arrow for horizontal sizing
                    ButtonsApp.window_move_active = 'N'
                    ButtonsApp.window_resize_active = 'Y'
                    ButtonsApp.mouse_listener_active2 = 'Y'
                #//TOP - North to South//
                elif (touch.pos[1] > Window.size[1] - 7 and touch.pos[0] < Window.size[0] - 7) and (touch.pos[1] > Window.size[1] - 7 and touch.pos[0] > 7):
                    window_resize_feature = 'n_s_t'
                    ButtonsApp.window_move_active = 'N'
                    ButtonsApp.window_resize_active = 'Y'
                    ButtonsApp.mouse_listener_active2 = 'Y'
                #//TOP RIGHT - Northeast to Southwest//
                elif touch.pos[1] > Window.size[1] - 7 and touch.pos[0] > Window.size[0] - 7:
                ...
                #//TOP LEFT - Northwest to Southeast//
                    ...
                    ButtonsApp.mouse_listener_active2 = 'Y'
                #//LEFT SIDE - West to East//
                elif (touch.pos[1] > 8 and touch.pos[0] < 7) and (touch.pos[1] < Window.size[1] - 8 and touch.pos[0] < 7):
    ...
    def mouse_listener(self): #this is running constantly in another thread while the Kivy App is running..
        ...
        while ButtonsApp.mouse_listener_active == 'Y':
            time.sleep(.0001) #you have to sleep here or the app doesn't launch??
            while ButtonsApp.mouse_listener_active2 == 'Y':
                if mouse2.position[0] != last_mouse_pos_x or mouse2.position[1] != last_mouse_pos_y:
                    ...
                    if touch_down_active == 'Y':
                        if window_resize_feature == 'disabled' and ButtonsApp.window_move_active == 'Y':
                        ...
                        elif ButtonsApp.window_resize_active == 'Y':
    ...
    def size_window(self, *args):
        ...
        if window_resize_feature == 'n_s_t':
            ...
            Window.last_screen_pos_top = Window.top
        #//TOP RIGHT - Northeast to Southwest//
        if window_resize_feature == 'ne_sw_t':
            Window.disable_on_restore = 'Y'
            Window.top = start_window_top + (last_mouse_pos_y - start_pos_y)
            Window.size = [start_window_width + (last_mouse_pos_x - start_pos_x), start_window_height - (last_mouse_pos_y - start_pos_y)]
            Window.last_screen_pos_left = Window.left
            Window.last_screen_pos_top = Window.top
        #//TOP LEFT - Northwest to Southeast//
        if window_resize_feature == 'nw_se_t':
            Window.disable_on_restore = 'Y'
            Window.top = start_window_top + (last_mouse_pos_y - start_pos_y)
            Window.left = start_window_left + (last_mouse_pos_x - start_pos_x)
            Window.size = [start_window_width - (last_mouse_pos_x - start_pos_x), start_window_height - (last_mouse_pos_y - start_pos_y)]
            ...
            Window.last_screen_pos_top = Window.top
        #//LEFT SIDE - West to East//
        if window_resize_feature == 'w_e_l':
            Window.disable_on_restore = 'Y'
            Window.left = start_window_left + (last_mouse_pos_x - start_pos_x)
            Window.size = [start_window_width - (last_mouse_pos_x - start_pos_x), start_window_height]
            Window.last_screen_pos_left = Window.left
            Window.last_screen_pos_top = Window.top
        #//BOTTOM LEFT - Northeast to Southwest//
        if window_resize_feature == 'ne_sw_b':
            Window.disable_on_restore = 'Y'
            Window.left = start_window_left + (last_mouse_pos_x - start_pos_x)
            Window.size = [start_window_width - (last_mouse_pos_x - start_pos_x), start_window_height + (last_mouse_pos_y - start_pos_y)]
            Window.last_screen_pos_left = Window.left
