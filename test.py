...
from win32gui import GetWindowText, GetForegroundWindow, FindWindow, SetActiveWindow, SetFocus, SetForegroundWindow, BringWindowToTop
...
class ButtonsApp(App, FloatLayout):
    ...
    def on_click(x, y, button, pressed):
        ...
        global aid
        global window_state
        if pressed and 'python.exe' in aid and ct == 'ButtonControl': #We have to look for 'python.exe' as a UIAutomation ID when the app icon in the taskbar is clicked.. not very elegant but it works..
    ...
    def get_control_info(self, instance):
        global ct
        global aid
        try:
            control_from_point = str(uiautomation.ControlFromPoint(mouse2.position[0], mouse2.position[1]))
            ctp = control_from_point.find('ControlType:')
            cnp = control_from_point.find('ClassName:')
            ct = control_from_point[ctp + 13:cnp - 1]
            ct = ct[0:ct.find(' ')]
            aip = control_from_point.find('AutomationId:')
            rp = control_from_point.find('Rect:')
            aid = control_from_point[aip + 13:rp - 1]
        except:
            pass
        Clock.unschedule(self.get_control_info)
